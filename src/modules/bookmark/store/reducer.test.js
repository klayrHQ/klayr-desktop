...
describe('Reducer: bookmarks(state, action)', () => {
  ...
  it(`should return wallets with added wallet if action.type is ${actionTypes.bookmarkAdded}`, () => {
    ...
    const state = { KLY: [wallet, wallet2] };
    const action = bookmarkAdded({ wallet: wallet3, token: 'KLY' });
    const changedState = bookmarks(state, action);
    expect(changedState.KLY[0]).toEqual(wallet3);
    expect(changedState.KLY[1]).toEqual(wallet);
    expect(changedState.KLY[2]).toEqual(wallet2);
  });
  ...
  it(`should return wallets with updated wallet if action.type is ${actionTypes.bookmarkUpdated}`, () => {
    ...
    expect(changedState.KLY[0]).toEqual(wallet);
    expect(changedState.KLY[1]).toEqual(updatedWallet);
  });
