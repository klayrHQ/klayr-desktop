...
describe('Reducer: watchList(state, action)', () => {
  ...
  it(`adds address to the watchlist state if the ${actionTypes.addedToWatchList} action is called`, () => {
    const action = {
      data: { address: 'klydwsyfmcko6mcd357446yatromr9vzgu7eb8y11' },
      type: actionTypes.addedToWatchList,
    };
    const updatedState = reducer(
      ['klydwsyfmcko6mcd357446yatromr9vzgu7eb8y12', 'klydwsyfmcko6mcd357446yatromr9vzgu7eb8y11'],
      action
    );

    expect(updatedState).toContain('klydwsyfmcko6mcd357446yatromr9vzgu7eb8y11');
    expect(updatedState).toContain('klydwsyfmcko6mcd357446yatromr9vzgu7eb8y12');
  ...
  it(`removes address from the watchlist state if the ${actionTypes.removedFromWatchList} action is called`, () => {
    ...
    expect(updatedState).toContain('klydwsyfmcko6mcd357446yatromr9vzgu7eb8y11');
    expect(updatedState).not.toContain('klydwsyfmcko6mcd357446yatromr9vzgu7eb8y12');
